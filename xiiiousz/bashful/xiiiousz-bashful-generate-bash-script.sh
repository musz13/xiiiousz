...
use_all_package=$(get_valid_yn "${XIIIOUSZ_COLOUR_01}Include all package?")
...
# Check if use_all_package is true (equals "y")
if [ "$use_all_package" == "y" ]; then
    # Do something if use_all_package is true
    echo "xiiiousz \"bashful\"" >>"$destination_script"
    echo "xiiiousz_bashful \"~\"" >>"$destination_script"
    echo "Including all packages..."
else
    # Do something else if use_all_package is not true
